...
int AdjvexInHeap[MAXNUM+1]; //记录图的节点在堆中的位置
...
typedef struct Heap{//用于生成最短路径树的堆结构
    ElemType node; //节点信息
    int vertex;    //节点在原图的下标
...
void HeapSortAdjust(Heap array[],int start,int end){ 
    ...
    for(int i = 2*start;i <= end;i *= 2){
        ...
        array[start] = array[i];
        AdjvexInHeap[array[start].vertex] = start;
        start = i; 
    } 
    array[start] = node; 
    AdjvexInHeap[array[start].vertex] = start;
...
void swap(Heap array[],int i,int j){
    AdjvexInHeap[array[i].vertex] = j;
    AdjvexInHeap[array[j].vertex] = i;
    Heap tmp; 
    tmp = array[i]; 
    array[i] = array[j]; 
    array[j] = tmp; 
...
Graph* Dijkstra(Graph *G,int startnode){
    int size = G->VertexNum;
    int n = size;
    Heap *array = InitMinHeaps(G,size);  //对堆结构进行初始化
    int start = 1;  //从堆顶开始进行Dijkstra
    array[startnode].distance = 0;  //把startnode距离赋值为0
    HeapSortAdjust(array,1,size);  //调整堆结构
    ...
    while(size > 0){
        ...
        while(p){  
            if(!visited[p->adjvex]){
                int i = AdjvexInHeap[p->adjvex];   //寻找该p->adjvex在堆的位置i
                int j = AdjvexInHeap[p->head];     //寻找该p->head在堆的位置j
                if(array[i].distance > p->weight + array[j].distance){ //比较p->adjvex在堆里的距离和堆顶节点到p->adjvex的距离
                    array[i].distance = p->weight + array[j].distance;
    ...
    size = n;
    //根据heap来创建最短路径树
    Graph *ShortTree = (Graph*)malloc(sizeof(Graph));
    ShortTree->VertexNum = size;
    ShortTree->EdgeNum = size-1;
    for(int k = 1;k <= ShortTree->VertexNum;k++){
        ShortTree->adjList[k].data = G->adjList[k].data; //节点结构与原图相同
        ShortTree->adjList[k].firstedge = NULL;
    }
    for(int k = 1;k <= ShortTree->VertexNum;k++){
        int i,j,w;
        ...
        ptrEdgeNode->head = j;
        ptrEdgeNode->weight = w;
        ptrEdgeNode->next = ShortTree->adjList[j].firstedge;
        ShortTree->adjList[j].firstedge = ptrEdgeNode;
        /*//无向图需要加上以下代码
        ptrEdgeNode = (EdgeNode*)malloc(sizeof(EdgeNode));
        ptrEdgeNode->adjvex = j;
        ptrEdgeNode->head = i;
        ptrEdgeNode->weight = w;
        ptrEdgeNode->next = ShortTree->adjList[i].firstedge;
        ShortTree->adjList[i].firstedge = ptrEdgeNode;*/
    }
    return ShortTree;
}
...
int main(){
    ...
    CreateALGraph(G); 
    Graph *ShortTree = Dijkstra(G,1);
    PrintGraph(ShortTree);
    return 0;
}